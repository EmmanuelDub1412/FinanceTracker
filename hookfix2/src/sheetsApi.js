...
// ── Normalize gapi errors into real Error objects with a readable message ──
// gapi client calls reject with an object shaped like
// { result: { error: { message, code } } } or a plain Response-like object,
// not a standard Error — so e.message is usually undefined. This wraps any
// gapi call so failures always carry a readable message.
const gapiCall = async (fn, label) => {
  try {
    return await fn();
  } catch (e) {
    const msg =
...
class SheetsDB {
  constructor() {
    ...
    this._inflight = {};   // { tabName: Promise } — dedupe concurrent reads
  }
  setSpreadsheetId(id) { this.spreadsheetId = id; this.cache = {}; this._inflight = {}; }
  // ── Initialize: create tabs + headers if missing ────────────────────────
  async init(spreadsheetId) {
    this.spreadsheetId = spreadsheetId;
    const meta = await gapiCall(
      () => window.gapi.client.sheets.spreadsheets.get({ spreadsheetId }),
      'Erreur lecture structure du sheet'
    );
    ...
    for (const [, tabDef] of Object.entries(TABS)) {
      ...
      if (!resp.result.values || resp.result.values.length === 0) {
        await gapiCall(
          ...
          `Erreur écriture en-têtes (${tabDef.name})`
        );
      }
    }
    this.cache = {};
  ...
  // Dedupes concurrent calls for the same tab so two near-simultaneous
  // reads (e.g. on app load) don't race and don't double-fail.
  ...
  async _findRowNum(tabName, id) {
    ...
    const resp = await gapiCall(
      ...
      `Erreur recherche ligne (${tabName})`
    );
  ...
  async delete(tabName, id) {
    ...
    const meta = await gapiCall(
      () => window.gapi.client.sheets.spreadsheets.get({ spreadsheetId: this.spreadsheetId }),
      'Erreur lecture structure du sheet'
    );
