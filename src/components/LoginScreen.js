  ...
  return (
    <div className="login-page">
      <div className="login-card" style={{ maxWidth: 400 }}>
        ...
        <div className="login-tag">{t('login.welcomeTitle')}</div>
        ...
        {(mode === 'signin' || mode === 'signup') && (
          <form onSubmit={handleSubmit} style={{ textAlign: 'left' }}>
            ...
            <div style={{ textAlign: 'center', marginTop: 14, fontSize: 12.5, color: 'var(--text2)' }}>
              {mode === 'signin' ? (
                <>{t('login.noAccount')} <span onClick={() => setMode('signup')} style={{ color: 'var(--g1)', cursor: 'pointer', fontWeight: 600 }}>{t('login.createAccount')}</span></>
              ) : (
                <>{t('login.hasAccount')} <span onClick={() => setMode('signin')} style={{ color: 'var(--g1)', cursor: 'pointer', fontWeight: 600 }}>{t('login.signin')}</span></>
              )}
            </div>

            <button type="button" className="btn btn-ghost" style={{ width: '100%', marginTop: 12, justifyContent: 'center' }}
              onClick={() => setMode('choice')}>{t('login.back')}</button>
          </form>
        )}
      </div>
    </div>
